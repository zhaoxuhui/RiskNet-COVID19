...
def load_rs_data(batch_size, resize=None, root='./rs'):
    ...
    # plt.imshow(to_img(data_train[0][0]))
    # plt.show()
...
# 训练模型
def trainUntil(train_iter, test_iter, net, optimizer, device, acc_threshold, max_epoch):
    logs = []
    ...
    while True:
        ...
        if epoch_counter <= max_epoch:
            ...
            if test_acc < acc_threshold:
                out_str = 'epoch {}, loss {:.4f}, train acc {:.4f}, test acc {:.4f}, target acc {:.4f}, time {:.2f} sec'.format(
                    epoch_counter,
                ...
                out_str = 'epoch {}, loss {:.4f}, train acc {:.4f}, test acc {:.4f}, time {:.2f} sec'.format(
                    epoch_counter,
                    train_l_sum / batch_count,
                    train_acc_sum / n,
                    test_acc,
...
if __name__ == '__main__':
    ...
    acc_threshold = 0.3  # 训练精度，直到大于等于才停止
    max_epoch = 20  # 最大训练次数
